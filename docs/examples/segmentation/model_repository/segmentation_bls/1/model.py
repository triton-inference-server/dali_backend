...
class TritonPythonModel:
    ...
    def execute(self, requests):
        ...
        for request in requests:
            ...
            original, preprocessed = run_inference(
                "dali_preprocessing", [in_encoded], ["original", "preprocessed"]
            )
            ...
            video_height = pb_utils.Tensor(
                "video_height", np.full((batch_size, 1), in_height, dtype=np.float32)
            )
            video_width = pb_utils.Tensor(
                "video_width", np.full((batch_size, 1), in_width, dtype=np.float32)
            )

            (segmented,) = run_inference(
                "dali_postprocessing",
                [original, probabilities, video_width, video_height],
                ["segmented"],
            )
